...
def upsample(filters, size, apply_dropout=False):
    result = tf.keras.Sequential()
    result.add(layers.UpSampling2D(size=2, interpolation='nearest'))
    result.add(layers.Conv2D(filters, size, strides=1,
...
def Generator():
    ...
    last = tf.keras.Sequential([
        ...
        layers.Conv2D(3, 5, strides=1,
                    padding='same',kernel_initializer=initializer,activation='relu')
    ])
...
def perceptual_loss(gen_output, target):
    gen = tf.keras.applications.vgg19.preprocess_input(gen_output * 255)
    tar = tf.keras.applications.vgg19.preprocess_input(target * 255)
    return tf.reduce_mean(tf.abs(feat_extractor(gen) - feat_extractor(tar)))
